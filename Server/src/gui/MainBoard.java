...
public class MainBoard {
    ...
    public MainBoard(Group group, Scene scene, ObjectOutputStream objectOutputStream, ObjectInputStream objectInputStream,
                     int playerX, int playerY,  int enemyX, int enemyY, Random random) {        //all the parameters derived from the start() method of main()
...
//        timeline.play();
    ...
    private void sendPlayerInformation() {
        ...
        playerInformation = new PlayerInformation(playerHp, playerMana, playerX, playerY); //invoke the constructor for assigning necessary values

        Platform.runLater(() -> {           //for updating GUI componet using non-GUI thread, here mana label and bar updating
            playerManaLabel.setText(String.valueOf(playerMana));
...
//            e.printStackTrace();
    ...
    private void sendShootSignal() {
        if (isRpg) {
            ...
            if (playerMana < rpgManaCost)       //if current mana is lower than cost
            ...
            if (pointerX >= enemyX&& pointerX <= enemyX + 150&& pointerY >= enemyY&& pointerY <= enemyY + 150) {
                try {                               //if the pointer can hit the enemy body(right click)
...
//                    e.printStackTrace();
            ...
            if (pointerX >= enemyX&& pointerX <= enemyX + 150&& pointerY >= enemyY&& pointerY <= enemyY + 150) {
                try {                               //left click, if the pointer can hit enemy body
                    objectOutputStream.writeInt(bulletHpReduction);     //send the hp reduction fr left clicks
                    objectOutputStream.flush();                         //clear
                } catch (IOException e) {
//                    e.printStackTrace();
                }
    ...
    private void catchSignal() {
        Thread thread = new Thread() {              //thread for retrieving the info
            @Override
            public void run() {
                do {
                    try {
                        ...
                        Platform.runLater(() -> {       //for updating GUI componet using non-GUI thread, here mana label and bar updating for enemy
...
//                        e.printStackTrace();
                ...
                } while (playerHp > 0&& enemyHp > 0);                   //contents of run() method will be continued until one of the players is dead
            }
        };
        thread.start();
    }
            ...
            Thread thread = new Thread() {
                ...
                public void run() {
                    ...
                    if (e.getButton().equals(MouseButton.PRIMARY)) {
                        ...
                        sendShootSignal();          //...and send the signal thereby
                    }
                        ...
                        sendShootSignal();      //...and send the signal
                    }
